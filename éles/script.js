    ...
    while (!placed && attempts < maxAttempts) {
        ...
        if (canPlace(row, col, ship.size, horizontal)) {
            for (let i = 0; i < ship.size; i++) {
                if (horizontal)
                    board[row][col + i] = { icon: ship.icon, id: ship.id };
        ...
        if (board[i][j] !== null) {
            ...
            cell.addEventListener("click", function () {
                ...
                if (isNormalGame) {
                    ...
                    // Csak ezután tárd fel a hajót
                    const shipId = this.getAttribute("data-ship-id");
                    revealShip(shipId);
                    this.classList.add("found");

                    // Adj hozzá bónusz lépést
                    remainingMoves += BONUS_MOVES;
                    lepesCounter.textContent = remainingMoves;

                    // Most már frissítheted a hajószámlálót
                    let currentValue = parseInt(counterSpan.textContent);
                    shipCounter(currentValue);
                }
            });
...
function GameOver() {
    if (gameEnded) return; // ha már vége volt, akkor semmit se csináljunk
    ...
    gameEnded = true; // csak most állítsd be, a végén!
}
